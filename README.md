# Tides 🌊 - The Ultimate Music Player

Tides is a modern, all-in-one music player for Windows that lets you play music from **YouTube, radio stations, and podcasts** in a clean, animated, Spotify-like interface. It features themes, animations, and a full playlist system for an immersive listening experience.

---

## Features

- **YouTube Search & Playlists**: Search for any video or playlist directly in the app.
- **Radio Streaming**: Stream popular radio stations like NPO Radio 1, Radio 538, Qmusic, and more.
- **Podcast Support**: Access and play popular podcasts.
- **Animated Controls**: Fully animated playback buttons and playlist effects.
- **Themes**: Switch between **Dexter 🩸**, **Forest 🌲**, and **Helldivers 2 🔫** themes.
- **Volume Control Slider**: Easily adjust the volume with a sleek slider.
- **Album Art Display**: See album art for tracks and radio streams.
- **History Tracking**: Every song you play is logged to a local history file.
- **Professional Updates Panel**: See all changes and new features in one place.

## Installation

### Windows
1. Download the latest release from the [Releases](https://github.com/flyingwumpusmtcell/TidesMusicDownload/releases/latest) page.
2. Run `Tides.exe`.
3. Allow access if prompted by Windows Defender SmartScreen.

---

## Usage

- Enter a **search query** in the top bar to find YouTube videos.
- Click **Themes 🎨** to switch the app’s theme.
- Use playback controls to play, pause, skip, and stop audio.
- Adjust the volume with the slider.
- Click **Updates 📝** to see all recent changes.

---

## Contributing

Contributions are welcome! If you have new features, bug fixes, or improvements:

1. Fork the repository.
2. Create a new branch: `git checkout -b feature/YourFeature`.
3. Commit your changes: `git commit -m 'Add new feature'`.
4. Push to the branch: `git push origin feature/YourFeature`.
5. Open a Pull Request.

---

## License

Tides is released under the **MIT License**. See [LICENSE](./LICENSE) for details.

---

## Support

For issues, feature requests, or feedback, please use the [GitHub Issues](https://github.com/USERNAME/TidesMusicDownload/issues) page.

---

Made with ❤️ for music lovers.
